...
loaduvm(pde_t *pgdir, char *addr, struct inode *ip, uint offset, uint sz)
{
  ...
  for(i = 0; i < sz; i += PGSIZE){
   ...
   /* int j = USERTOP - PGSIZE;
    if((pte = walkpgdir(pgdir, addr+j, 0)) == 0)
      panic("loaduvm: address should exist");
    pa = PTE_ADDR(*pte);
    if(readi(ip, (char*)pa, offset+j, PGSIZE) != PGSIZE)
      return -1;*/
  return 0;
}
...
allocuvm(pde_t *pgdir, uint oldsz, uint newsz)
{
  char *mem;
  uint a;
  if(newsz > (uint)PGROUNDUP(proc->tf->esp) && proc->stack  != 0 && proc->sa == 0)
    return 0;

  if(PGROUNDUP(proc->tf->esp) < proc->sz && proc->stack != 0 )
    return 0;
  if(newsz < oldsz)
    return oldsz;
...
int
clearPage(pde_t *pgdir, uint addr)
{
	pte_t *pte;
