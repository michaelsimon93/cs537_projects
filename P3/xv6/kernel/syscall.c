...
fetchint(struct proc *p, uint addr, int *ip)
{
  if(addr >= p->sz || addr+4 > p->sz) {
    if (addr < (uint)PGROUNDDOWN(p->tf->esp)) return -1;
  }
  *ip = *(int*)(addr);
  return 0;
}
...
fetchstr(struct proc *p, uint addr, char **pp)
{
  ...
  if(addr >= p->sz || (addr <= 0x1000 && p->pid >= 2)) {
    if (addr < (uint)PGROUNDDOWN(p->tf->esp)) return -1;
  }
  *pp = (char*)addr;
  ep = (char*)p->sz;
...
argptr(int n, char **pp, int size)
{
  ...
  if(argint(n, &i) < 0)
    return -1;
  if((uint)i >= proc->sz || (uint)i+size > proc->sz || ((uint)i <=0x1000 && proc->pid >=2)) {
    if ((uint)i < (uint)PGROUNDDOWN(proc->tf->esp)) return -1;
  }
  *pp = (char*)i;
  return 0;
}
