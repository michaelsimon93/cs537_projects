...
#define MAP_1 0x00FFFFFF
#define MAP_2 0xFF000000
...
bmap(struct inode *ip, uint bn)
{
  ...
  if(bn < NDIRECT){
    ...
    /*
     * Pretty confident this is needed
     */
    if  (ip->type == T_CHECKED) {
    	/*
	 * check address and truncate
	 */
    	addr = addr & MAP_1;
    }
    return addr;
  ...
  st->size = ip->size;
	...
	if (ip->type == T_CHECKED) {
		...
		for (i = 0; i < (NDIRECT + 1); i++) {
			...
			tmp = 0;
			addr = ip->addrs[i];
			if ( i == NDIRECT) { //is indirect block

				bp = bread(ip->dev, ip->addrs[i]);
				a = (uint *)bp->data;
				int q, chk = 0;
				for (q = 0; q < BSIZE/sizeof(uint); q++) {
					chk = a[q];
					chk = chk & MAP_2;
					chk = chk >> 24;
					tmp = (uchar)chk;
...
readi(struct inode *ip, char *dst, uint off, uint n)
{
  ...
  for(tot=0; tot<n; tot+=m, off+=m, dst+=m){
	...
	if (ip->type == T_CHECKED) {
		...
		if (bn < NDIRECT) {
			//get checksum
			chksm = ip->addrs[bn] & MAP_2;
			//have to shift so 0x??000000 -> 0x000000??
			chksm = chksm >> 24;
		}
		//have to check indirect block if exists
		//copied from bmap
		
		bn -= NDIRECT;
		if(bn < NINDIRECT) {
			//indirect block
			cbp = bread(ip->dev, ip->addrs[NDIRECT]);
			a = (uint*)cbp->data; //get data from disk
			chksm = a[bn] & MAP_2;
			chksm = chksm >> 24;
			brelse(cbp);
		}
  ...
  for(tot=0; tot<n; tot+=m, off+=m, src+=m){
	...
	if (ip->type == T_CHECKED) {
		...
		if (bn < NDIRECT) {
			...
			uint chksm = (uint)chk;
			chksm = chksm << 24;
			ip->addrs[bn] = ip->addrs[bn] & MAP_1;
			ip->addrs[bn] = ip->addrs[bn] | chksm;
